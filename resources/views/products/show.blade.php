...
@section('content')
  <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
    <div class="md:col-span-2 bg-white p-6 rounded shadow">
      <div class="mb-4 rounded overflow-hidden h-64">
        @if($barang->image_url)
      ...
      <div class="mt-4">
        <div class="flex items-center space-x-2">
          ...
          @if(session('user_id'))
            <form action="{{ url('/product/'.$barang->id_barang.'/like') }}" method="POST" class="ml-2">
              @csrf
              <button type="submit" title="Like" aria-label="Like" class="focus:outline-none">
                @if(!empty($liked))
                  <!-- filled heart -->
          ...
          @else
            <a href="{{ route('login') }}" class="ml-2 text-pink-600">
